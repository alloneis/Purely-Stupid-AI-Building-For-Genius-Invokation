...
export interface ResetStateOption extends Omit<NotifyOption, "mutations"> {
  readonly deferNotify?: boolean;
  readonly pure?: boolean;
}
export interface DeferredNotifyOption {
  readonly deferNotify?: boolean;
}
...
export interface MutatorConfig {
  ...
  /**
   * Patch  发送策略
   * - "immediate": 等同于旧行为，在需要时立即 notify
   * - "deferred": 仅累计 patch，在外层 notify/notifyAndPause 时统一 flush
   */
  readonly patchDelivery?: "immediate" | "deferred";
...
export type StatePatch = ExposedMutation;
...
export class StateMutator {
  ...
  private _patchesToBeNotified: StatePatch[] = [];
  private _allRecordedPatches: StatePatch[] = [];
  ...
  resetState(
    ...
    if (notifyOpt?.pure ?? this._silent) {
      this.stagePatches(withMutations.exposedMutations, {
    ...
    this.notify({
      force: notifyOpt?.force,
      canResume: notifyOpt?.canResume,
  ...
  private stagePatches(
    patches: readonly StatePatch[],
    opt: { notifyImmediately?: boolean } = {},
  ) {
    if (patches.length === 0) {
      return;
    }
    this._patchesToBeNotified.push(...patches);
    this._allRecordedPatches.push(...patches);
    

    const patchDelivery = this.config.patchDelivery ?? "immediate";
    const shouldNotify =
      !this._silent &&
      (opt.notifyImmediately ?? true) &&
      patchDelivery === "immediate";
    if (shouldNotify) {
      this.notify();
    }
  }
  ...
  /**
   * 在外部动作逻辑中主动记录 patch（例如用于纯模拟器
   */
  recordPatches(
    patches: readonly StatePatch[],
    opt: { notifyImmediately?: boolean } = {},
  ) {
    this.stagePatches(patches, opt);
  }

  /**
   * 当前尚未 flush 到外部的 patch 队列（用于纯模拟）
   */
  getStagedPatches(): readonly StatePatch[] {
    return [...this._patchesToBeNotified];
  }

  /**
   *  提取并清空未 flush 的 patch 队列（用于纯模拟）
   */
  drainStagedPatches(): StatePatch[] {
    const patches = [...this._patchesToBeNotified];
    this._patchesToBeNotified = [];
    return patches;
  }
   ...
   *查看全部 patch 历史（不会清空）
   */
  getAllRecordedPatches(): readonly StatePatch[] {
    return [...this._allRecordedPatches];
  }

  private createNotifyInternalOption(opt: NotifyOption): InternalNotifyOption {
    const inlinePatches = opt.mutations ?? [];
    const queuedPatches = this._patchesToBeNotified;
    const result = {
      state: this.state,
      canResume: opt.canResume ?? false,
      stateMutations: this._mutationsToBeNotified,
      exposedMutations: [...queuedPatches, ...inlinePatches],
    };
    this._mutationsToBeNotified = [];
    this._patchesToBeNotified = [];
    return result;
  ...
  notify(opt: NotifyOption = {}) {
    if (this._silent) {
      const inlineMutations = opt.mutations ?? [];
      if (inlineMutations.length > 0) {
        this.stagePatches(inlineMutations, { notifyImmediately: false });
      }
      this._mutationsToBeNotified = [];
      this._mutationsToBePause = [];
      return;
  ...
  private executeInlineSkill<Arg>(
    ...
    this.resetState(newState, innerNotify, { pure: pureMode });
  ...
  /**
   * 杞婚噺鎵ｈ鍘熻锛氬彧鏀瑰姩鐘舵€佸苟璁板綍 patch锛屼笉鐩存帴璇曞浘鍊熷彛鍘绘帹杩涘叾浠栨父鎴忔祦绋?
   * 鐢ㄤ簬瑙ｈ€︿腑鐨勭函妯℃嫙鍜屽皬姝ュ喅绛栬瘯楠屻€?
   */
  applyPureDamage(
    targetState: CharacterState,
    damage: number,
    sourceState: AnyState = targetState,
  ): { oldHealth: number; newHealth: number } {
    const oldHealth =
      typeof targetState.variables.health === "number"
...
- For attachments, use createAttachment instead.`,
  ...
  async reroll(who: 0 | 1, times: number, opt: DeferredNotifyOption = {}) {
  ...
  // this.notify() 也改成可 defer（由外层统一 flush）。 
  async switchHands(
    who: 0 | 1,
    opt: DeferredNotifyOption = {},
...
/**
 * 用于规则纯模拟的 mutator：只修改状态，不触发外部通知。
 * 支持将变更收集为 patch 数组，供上层统一处理。
 */
export class PureStateMutator {
  private readonly mutator: StateMutator;
  private readonly patches: PureStatePatch[] = [];

  constructor(initialState: GameState, logger?: IDetailLogger) {
  // GameState 包含 definition 函数等无法被 structuredClone 的成员。
  // applyMutation 基于 immer 的 produce，会返回新状态，不会原地污染输入对象。
  ...
  applyDamage(targetId: number, damage: number) {
    const target = getEntityById(this.mutator.state, targetId);
    if (!target || target[StateSymbol] !== "character") {
      throw new GiTcgDataError(`Character ${targetId} not found.`);
    }
    const character = target as CharacterState;
    const oldHealth =
      typeof character.variables.health === "number"
        ? character.variables.health
        : 0;
    const newHealth = Math.max(0, oldHealth - Math.max(0, damage));
    this.mutator.mutate({
      type: "modifyEntityVar",
      state: character,
      varName: "health",
  ...
  getPatches(): readonly PureStatePatch[] {
    return [...this.patches];
  }

  drainPatches(): PureStatePatch[] {
