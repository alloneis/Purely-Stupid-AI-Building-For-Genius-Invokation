...
enableMapSet();
// Hot simulation loops should not pay deep-freeze cost.
  ...
  switch (m.type) {
    ...
    case "stepId": {
      ...
      return {
        ...
        iterators: {
          ...state.iterators,
          id: state.iterators.id - 1,
        },
      };
    }
    case "changePhase": {
      if (state.phase === m.newPhase) {
        return state;
      }
      return {
        ...state,
    ...
    case "switchTurn": {
      return {
        ...state,
        currentTurn: flip(state.currentTurn),
      };
    }
    case "setWinner": {
      if (state.winner === m.winner) {
        return state;
      }
      return {
        ...state,
    ...
    case "switchActive": {
      ...
      if (player.activeCharacterId === m.value.id) {
        return state;
      }
      const players = [...state.players] as [PlayerState, PlayerState];
      players[m.who] = {
        ...player,
        activeCharacterId: m.value.id,
      };
      return {
        ...state,
        players,
    ...
    case "resetDice": {
      ...
      players[m.who] = {
        ...players[m.who],
        dice: sortDice(state.players[m.who], m.value),
      };
      return {
        ...state,
        players,
    ...
    case "pushRoundSkillLog": {
      ...
      const nextRoundSkillLog = new Map(player.roundSkillLog);
      const prev = nextRoundSkillLog.get(key) ?? [];
      nextRoundSkillLog.set(key, [...prev, m.skillId]);
      const players = [...state.players] as [PlayerState, PlayerState];
      players[who] = {
        ...player,
        roundSkillLog: nextRoundSkillLog,
      };
      return {
        ...state,
        players,
    ...
    case "pushDelayingEvent": {
      return {
        ...state,
        delayingEventArgs: [...state.delayingEventArgs, [m.eventName, m.eventArg]],
      };
    }
    case "clearDelayingEvent": {
      if (state.delayingEventArgs.length === 0) {
        return state;
      }
      return {
        ...state,
