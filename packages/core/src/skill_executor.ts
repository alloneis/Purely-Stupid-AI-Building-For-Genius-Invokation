...
export class SkillExecutor {
  ...
  private finalizeSkillPreview(skillInfo: SkillInfo, arg: GeneralSkillArg): void {
    if (this.state.phase === "gameEnd") {
      return;
    }
    let emittedEvents = [...this.executeSkill(skillInfo, arg)];
    void this.mutator.notifyAndPause();
    ...
    do {
      ...
      for (const event of emittedEvents) {
        const [name, eventArg] = event;
        if (name === "onDamageOrHeal" && eventArg.isDamageTypeDamage()) {
          if (eventArg.damageInfo.causeDefeated) {
            const zeroHealthEventArg = new ZeroHealthEventArg(
              eventArg.onTimeState,
              eventArg.damageInfo,
              eventArg.option,
            );
      ...
      for (const damageEventArg of damageEventArgs) {
        if (damageEventArg.damageInfo.causeDefeated) {
          criticalDamageEvents.push(damageEventArg);
        } else {
          safeDamageEvents.push(damageEventArg);
        }
      }
      ...
      emittedEvents = [];
      for (const zeroHealthEventArg of zeroHealthEventArgs) {
        emittedEvents.push(
          ...this.handleEventShallow(["modifyZeroHealth", zeroHealthEventArg]),
        );
      }
    ...
    if (
      ...
      const ch = getEntityById(this.state, skillInfo.caller.id) as CharacterState;
      this.mutate({
        type: "pushRoundSkillLog",
        caller: skillInfo.caller as CharacterState,
    ...
    this.handleEventPreview(...nonDamageEvents);
    for (const damageEventArg of safeDamageEvents) {
      this.handleEventPreview(["onDamageOrHeal", damageEventArg]);
    }
    for (const damageEventArg of criticalDamageEvents) {
      this.handleEventPreview(["onDamageOrHeal", damageEventArg]);
    }
    if (criticalDamageEvents.length > 0) {
      this.mutator.log(
        DetailLogType.Other,
        "Skip forced chooseActive in preview mode",
      );
    }
  }

  private handleEventPreview(...events: EventAndRequest[]) {
    const queue = [...events];
    while (queue.length > 0) {
      ...
      if (arg instanceof RequestArg) {
        this.mutator.log(
          DetailLogType.Other,
          `Skip request event ${name} in preview mode`,
        );
        continue;
      }
      ...
      const emitted = this.handleEventShallow(event);
      queue.push(...emitted);
    }
  }
  ...
  async finalizeSkill(
    ...
    if (this.config.preview) {
      this.finalizeSkillPreview(skillInfo, arg);
      return;
    }
    let emittedEvents = [...this.executeSkill(skillInfo, arg)];
  ...
  async handleEvent(...events: EventAndRequest[]) {
    if (this.config.preview) {
      this.handleEventPreview(...events);
      return;
    }
    for (const event of events) {
      const [name, arg] = event;
      ...
      if (name === "requestReroll") {
        ...
        await this.mutator.reroll(arg.who, arg.times, { deferNotify: true });
  ...
  /**
   * Pure in-memory skill execution branch.
   * This only executes the skill body and synchronous post-skill bookkeeping.
   * Event cascade should be handled by caller through a stability loop.
   */
  executeSkillSync(
    skillInfo: SkillInfo,
    arg: GeneralSkillArg,
    ...
    if (
      ...
      if (
        skillInfo.definition.initiativeSkillConfig.gainEnergy &&
        ch.variables.alive &&
        !ch.definition.specialEnergy
      ) {
        const currentEnergy = ch.variables.energy;
        const newEnergy = Math.min(currentEnergy + 1, ch.variables.maxEnergy);
        this.mutate({
  ...
  /**
   * Pure in-memory event handling branch.
   * It only executes non-request events and returns newly emitted events.
   * Any request event means the caller must provide external decisions first.
   */
  handleEventSync(...events: EventAndRequest[]): EventAndRequest[] {
    const emittedEvents: EventAndRequest[] = [];
    for (const event of events) {
      const [name, arg] = event;
      if (arg instanceof RequestArg) {
        throw new GiTcgCoreInternalError(
          `Sync event execution does not support request event ${name}`,
        );
      }
      if (name === "onDamageOrHeal" || name === "onReaction") {
        this.mutate({
      ...
      emittedEvents.push(...this.handleEventShallow(event));
    }
    return emittedEvents;
  }
  ...
  static executeSkillSync(
    mutator: StateMutator,
    skill: SkillInfo,
    arg: GeneralSkillArg,
  ): readonly EventAndRequest[] {
    const executor = new SkillExecutor(mutator, { preview: false });
    return executor.executeSkillSync(skill, arg);
  }

  static executeEventSync(
    mutator: StateMutator,
    event: EventAndRequest,
  ): EventAndRequest[] {
    const executor = new SkillExecutor(mutator, { preview: false });
    return executor.handleEventSync(event);
  }
}
