...
function getAgent(searchConfig?: IsmctsWorkerRequest["searchConfig"]) {
  ...
  const agent = new ISMCTSAgentCtor(normalized);
...
workerScope.onmessage = (event: MessageEvent<IsmctsWorkerRequest>) => {
  ...
  try {
    ...
    if (typeof ismcts.getBestActionWithDebug === "function") {
      ...
      candidates = result.debug.candidates.map((entry) => ({
        action: compactActionForResponse(entry.action),
        visits: entry.visits,
        meanValue: entry.meanValue,
      }));
      top3 = result.debug.top3.map((entry) => ({
        action: compactActionForResponse(entry.action),
        visits: entry.visits,
        meanValue: entry.meanValue,
      }));
